                  ...
                  <Row type="flex" justify="start">
                    <Col span="8">
                      <FormItem label="商家" class="ivu-form-item-required">
                        <Input v-model="product.merchantDTO.merchantName" disabled style="width:60%"></Input>
                      </FormItem>
                    </Col>
                    <Col span="8">
...
export default {
  ...
  methods: {
        ...
        .then(
          response => {
            if (response.data.body.status.statusCode == 0) {
              ...
              this.product.productType = product.productType;//初始化商品类型
              //电子卡券商品有效期初始化
              ...
              if(product.merchantDTO){
                this.product.merchantDTO = {
                  merchantUuid: product.merchantDTO.merchantUuid,
                  merchantName: product.merchantDTO.merchantName
                }
              }
              ...
              this.inquiryProductFreight(product.merchantDTO.merchantUuid);
              this.inquiryProductDelivery(product.merchantDTO.merchantUuid);
            } else {
              this.$Message.error(response.data.body.status.errorDesc);
            }
          },
    ...
    //查询运费模板
    inquiryProductFreight(merchantUuid) {
      var options = {
        merchantDTO:{
