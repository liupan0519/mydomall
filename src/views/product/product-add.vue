                  ...
                  <Row type="flex" justify="start">
                    <Col span="12">
                      ...
                      <RadioGroup v-model="product.productType" size="large" type="button">
                          <Radio label="1">实物商品(物流发货)</Radio>
                          <Radio label="2">虚拟商品(无需物流)</Radio>
                          <Radio label="3">电子卡券(线下核销)</Radio>
                      </RadioGroup>
                      </FormItem>
                     </Col>
                  </Row>
                  <Row type="flex" justify="start">
                    <Col span="8">
                      <FormItem label="商家" class="ivu-form-item-required">
                        <Select placeholder="输入商家名称搜索" @on-change="changeMerchant" v-model="selectedMerchant" style="width:50%"  filterable remote :remote-method="searchMerchant" :loading="merchantLoading" clearable>
                          <Option
  ...
  methods: {
    ...
    addok(name) {
      ...
      this.$refs[name].validate(valid => {
        if (valid) {
          if(!_this.selectedMerchant){
              this.$Message.error('商家必选');
                return false;
          }
    ...
    //商家改变
    changeMerchant(merchantUuid){
      this.inquiryProductFreights(merchantUuid);
      this.inquiryProductDelivery(merchantUuid);
    },
    //查询运费模板
    inquiryProductFreights(merchantUuid) {
    ...
    //查询同城配送模板
    inquiryProductDelivery(merchantUuid) {
      if(!merchantUuid) return
      let that = this;
      this.$http
        .post("/admin/inquiryProductDelivery", this.common.request({
          merchantDTO:{
            merchantUuid: merchantUuid
          }
        }))
