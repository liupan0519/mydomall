...
<template>
	<div>
			...
			<Card>
				...
				<Row>
					<div>
						...
						<Select v-model="searchMerchant" placeholder="选择商家" style="width:180px" filterable remote :remote-method="inquiryMerchant"
						 :loading="merchantLoading" clearable>
							<Option v-for="item in merchants" :value="item.merchantUuid" :key="item.merchantUuid">{{item.merchantName}}
							</Option>
						</Select>
                ...
                <span slot="close">上架</span>
						...
						<Button class="f_r" icon="plus" type="warning" @click="exportFile()">导入商品</Button>
					</div>
				</Row>
		...
		<div>
			<Modal title="导入商品" v-model="exportFileModal">
			...
			<Modal v-model="approveModel" ref="approveModel" title="商品审核">
				你确认通过该商品上架申请吗 ?
				<div slot="footer">
			...
			<Modal v-model="rejectModel" ref="rejectModel" title="商品审核">
				<Input type="textarea" v-model="verifyMsg" :maxlength="500" :rows="5" placeholder="请输入拒绝理由"></Input>
				<div slot="footer">
				...
				this.productColumn = [
					...
					},
					{
						...
						render: (h, params) => {
							return h('span', {
								style: {
									display: params.row.verifyStatus == '2' ? '' : 'none'
								}
