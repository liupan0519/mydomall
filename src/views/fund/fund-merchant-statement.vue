...
<template>
    <div>
            ...
            <Card>
                ...
                <Row>
                <Col span="6">
                    <FormItem label="商家名称">{{merchant.merchantName}}</FormItem>
                </Col>
                <Col span="6">
                    <FormItem label="商家地址">{{merchant.merchantAddress}}</FormItem>
                </Col>
                <Col span="6">
                    <FormItem label="联系人姓名">{{merchant.contactName}}</FormItem>
                </Col>
                <Col span="6">
                    <FormItem label="手机号码">{{merchant.mobileNo}}</FormItem>
                </Col>
                <Col span="6">
                    <FormItem label="资金余额">
        <font color="red" size="6">{{merchant.availableBalance||0}}</font>
                    </FormItem>
                </Col>
                </Row>
            </Card>
...
export default {
    ...
    methods: {
        ...
        inquiryMerchant(merchantUuid){
            ...
            this.$http.post("/admin/inquiryMerchant", this.common.request(searchOptions))  
            .then(response => {
                this.loading = false;
        ...
        searchStatement(startIndex, pageSize){
            ...
            this.$http.post("/admin/searchMerchantStatement", this.common.request(searchOptions))  
            .then(response => {
                this.loading = false;
                if(response.data.body.status.statusCode==0){
                    var data = response.data.body.data;
