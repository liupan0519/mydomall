		...
		<view class="voucher" v-for="voucher in vouchers">
			<view class="voucher-text">
				...
				<view class="voucher-divider">
					<u-divider>{{voucherMsg.voucherPrompt}}</u-divider>
				</view>
				...
				<view class="voucher-btn">
					<u-button @click="saveVoucher(voucher.qrCodeUrl,voucher.voucherCode)" type="error" :ripple="true" shape="square" size="medium" plain>{{voucherMsg.saveVoucher}}</u-button>
				</view>
				...
				uni.downloadFile({
					...
					success (res) {
						uni.saveImageToPhotosAlbum({
							...
							success() {
								_this.$api.msg(_this.voucherMsg.saveSuccess);
							},
							fail() {
								_this.$api.msg(_this.voucherMsg.saveFail);
							}
						});
					},
					fail (res) {
