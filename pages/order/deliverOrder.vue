<template>
	<view class="container">
		...
		<view v-if="order.deliveryType=='1'">
			...
			<button class="add-btn" @click="save">{{i18n.confirmBtn}}</button>
		</view>
		<!-- 同城配送 -->
		<view v-if="order.deliveryType=='2'">
			<view class="row" v-if="shoppers.length>0">
				<text class="tit">{{i18n.courier.shopper}}</text>
			...
			<view v-else>
				<view class="empty">
					{{i18n.courier.shopperEmpty}}
				</view>
			</view>
			<button  v-if="shoppers.length>0" class="add-btn" @click="save">{{i18n.confirmBtn}}</button>
			<button  v-if="shoppers.length==0" class="add-btn" @click="back">{{i18n.backBtn}}</button>
		</view>
		<!-- 门店自提 -->
		<view v-if="order.deliveryType=='3'">
			<view class="row">
				<text class="tit">{{i18n.address.deliveryLongitude}}</text>
				<text class="input">{{order.deliveryName}}</text>
			</view>
			<view class="row">
				<text class="tit">{{i18n.address.extractTel}}</text>
	...
	export default {
		...
		onLoad(options) {
			uni.setNavigationBarTitle({
				title: this.i18n.order.deliverOrderTitle
			})
